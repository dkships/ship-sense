...
def test_partition_models_reports_batch_direct_and_local_paths():
    """The direct-lane example is muse-spark-1.1 on purpose: Meta ships no batch
    endpoint at all, so it is live-only for a structural reason. gpt-5.6-sol used
    to sit here and had to move on 2026-07-31 when the Batch API started accepting
    it — pick a model whose lane cannot flip under a provider rollout."""
    plan = batch.partition_models(["claude-haiku-4-5", "muse-spark-1.1", "mock-naive"])
    assert plan == {
        "batch": ["claude-haiku-4-5"],
        "direct": ["muse-spark-1.1"],
        "local": ["mock-naive"],
    }


def test_prepare_cli_keeps_paths_on_stdout_and_summary_on_stderr(
        monkeypatch, capsys):
    monkeypatch.setattr(batch, "prepare", lambda *args, **kwargs: [Path("manifest.json")])
    monkeypatch.setattr(batch, "partition_models", lambda models: {
        "batch": ["batch-model"],
        "direct": ["direct-one", "direct-two"],
        "local": ["mock-naive"],
    })
    monkeypatch.setattr(sys, "argv", [
        "batch", "prepare", "--models", "batch-model", "direct-one",
        "direct-two", "mock-naive", "--run-id", "test",
    ])

    batch.main()

    captured = capsys.readouterr()
    assert captured.out == "manifest.json\n"
    assert 'Direct MODELS="direct-one direct-two mock-naive"' in captured.err
...
# Client timeouts
# --------------------------------------------------------------------------- #
# Regression guard for 2026-09-01: a batch run stalled indefinitely because the
# SDK clients src.batch builds for itself carried no request timeout. The machine
# slept mid-run, and on wake the in-flight status poll sat on a socket that had
# died hours earlier. notes/run_batch.py checks its own wall clock only between
# stages, so nothing bounded the call -- the batch had finished server-side while
# the driver waited on a dead connection.
...
def test_default_clients_carry_a_request_timeout(monkeypatch):
    """Every self-built client bounds its requests, so a dead socket raises."""
    pytest.importorskip("anthropic")
    pytest.importorskip("openai")
    _dummy_provider_keys(monkeypatch)
    expected = batch.BATCH_HTTP_TIMEOUT_S

    assert batch._anthropic_client().timeout == expected
    assert batch._openai_client().timeout == expected


def test_gemini_client_timeout_is_milliseconds(monkeypatch):
    """google-genai takes HttpOptions.timeout in MILLISECONDS, unlike the other
    two SDKs, which take seconds. Passing seconds there would set a 2-minute
    budget to 120ms and fail every call."""
