...
# --------------------------------------------------------------------------- #
# One version token per label: "4.6", "5", "3.5", or Moonshot's "K3" shape.
# Guarded on both sides so "Flash-Lite", "GPT-5.6" (hyphen approach) and plain
# words never half-match.
_VERSION_TOKEN = re.compile(r"(?i)(?<![a-z0-9.])k?(\d+(?:\.\d+)*)(?![a-z0-9.])")
...
--anthropic:#c15f3c;--openai:#10a37f;--google:#3d6fc4;--xai:#8e3b78;--meta:#526200;--moonshot:#d5a33d;
...
background:conic-gradient(from 210deg,var(--anthropic),var(--openai),var(--google),var(--xai),var(--meta),var(--moonshot),var(--anthropic))}
...
.gcard .pill{display:inline-flex;align-items:center;gap:.45rem;font:700 .62rem/1 var(--mono);
...
.gcard .gdelta{font:.76rem/1.5 var(--mono);color:var(--mut);margin-top:.7rem;
...
def _retired_row(m: dict, succ_label: str | None, bank_n: int | None) -> str:
    """A previous-generation model as a full-detail table row: same columns as
    the main board, rank cell dashed, successor named under the lab."""
    released = escape(m["released"]) if m.get("released") else "&mdash;"
    color = _provider_color(m.get("provider"))
    sc = m["score"]
    provider = _provider_name(m.get("provider"))
    sub = (f'{escape(provider)} &middot; replaced by {escape(succ_label)}'
           if succ_label else escape(provider))
    dim_cells = ""
    for d in DIMENSIONS:
        c = m[d]
        dim_cells += (f'<td class="dim">{_bar(c["value"], c["lo"], c["hi"], 1.0, "var(--dim-bar)")}'
    ...
    return (f'<tr>'
            ...
            f'<td class="model"><span class="dot" style="background:{color}"></span>'
            f'<span class="mname"><span class="label">{escape(m["label"])}</span>'
            f'<span class="provider">{sub}</span></span></td>'
            f'<td class="rel">{released}</td>'
            f'{_price_cell(m)}'
            f'<td class="score">{_bar(sc["value"], sc["lo"], sc["hi"], 100.0, color)}'
            f'<span class="num big">{sc["value"]:.1f}</span>'
...
def _matchup_card(p: dict) -> str:
    ...
    holm = p.get("holm_p")
    holm_txt = ("" if holm is None or not p["decisive"]
                else f' (Holm p {holm:.3g})')
    qual = _qualifier(p)
    delta_line = ("&#916; not paired" if p["delta"] is None
                  else f'&#916; {_pair_delta_text(p)}'
                  + (f' &middot; {qual}{holm_txt}' if qual else ""))
    return (f'<div class="gcard" style="--gink:{ink}">'
            f'<span class="pill {pill_cls}">{pill}</span>'
            f'{vrow(curr, "current", p["winner"] == "curr")}'
            f'{vrow(prev, "previous", p["winner"] == "prev")}'
            f'<div class="gdelta">{delta_line}</div>'
            f'</div>')


def _generations_section(pairs: list[dict], bank_n: int | None) -> str:
    """The page's Generations section: matchup cards (the verdicts), the
    dumbbell chart (the shape), and full rows for every retired model (the
    detail)."""
    if not pairs:
        return ""
    cards = f'<div class="gcards">{"".join(_matchup_card(p) for p in pairs)}</div>'
    chart = _generations_svg(pairs)
    chart_block = (f'<div class="fieldwrap">{chart}<p class="fcap">{_GENS_CAPTION}</p></div>'
                   if chart else "")
    rows = "".join(_retired_row(p["prev"], p["curr"]["label"], bank_n) for p in pairs)
...
successor, the outgoing model retires here automatically. It's still scored on the same bank
in the same run, replayed against its replacement item by item, and the paired test decides
whether the successor is an upgrade.</p>
{cards}
{chart_block}
...
<p class="note">Retired models keep their scores, their ledger rows, and their head-to-head
records; only board placement changes. Succession is automatic: a ranked model in the same
line with a higher version retires its predecessor. Renamed lines are declared in the
public model registry (<code>models.yaml</code>, <code>superseded_by</code>).</p>
</section>"""
...
def _generations_markdown(pairs: list[dict]) -> list[str]:
    ...
    lines += ["| Previous | Current | Paired Δ (95% CI) | Verdict |",
              "|---|---|---|---|"]
    for p in pairs:
        prev, curr = p["prev"], p["curr"]
    ...
    lines += ["",
              ...
              "full rows for retired models are on the "
...
def render_html(ledger: dict, png_b64: str | None = None) -> str:
    ...
    gen_pairs = _generation_pairs(run["models"], previous, pairwise)
    ranked = rank_with_ties(current)
    eligible = _eligible_rows(ranked)
...
input/output tokens.{gens_board_note}{coverage_note}</p>
</section>

{_generations_section(gen_pairs, bank_n)}
...
def render_markdown(ledger: dict) -> str:
    ...
    lines.append(f"<sub>Run {date} · {_bank_label(run['bank'])} "
                 ...
                 f"$/M = list price per 1M input/output tokens{gens_note}.</sub>")
